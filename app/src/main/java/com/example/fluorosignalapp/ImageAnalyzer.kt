...
class ImageAnalyzer {
     ...
     * @param useGaussianBlur 是否使用高斯模糊
     * @param useMedianBlur 是否使用中值滤波（更强去噪）
     * @param ignoreExtreme 是否过滤极端像素
     * @param extremeQuantile 极端值分位数 (lower, upper) 如 (0.01, 0.99)
     * @param normalize 是否归一化绿色通道
     * @param roiRect 可选的感兴趣区域矩形 Rect(x, y, width, height)
     * @param analyzeAllChannels 是否同时分析RGB三通道
     * @return AnalysisResult 包含增强的分析指标
     */
    suspend fun analyze(
        imageFile: File,
        ...
        try {
            ...
            // 2. 确保图像是 BGR 三通道格式，并转换为 CV_8U 类型
            if (image.channels() == 1) {
                // 灰度图转 BGR
                val bgrImage = Mat()
    ...
    private fun calculateChannelStats(
        ...
        Log.d(TAG, "calculateChannelStats - Channel type: ${channel.type()}, depth: ${channel.depth()}, channels: ${channel.channels()}")

        // 确保 Mat 是单通道 CV_8U 类型
        ...
        try {
            ...
            if (workingChannel.depth() != CvType.CV_8U) {
                ...
                Log.d(TAG, "Converted to CV_8U type")
            }
            ...
            Log.d(TAG, "Stats calculated - Mean: $mean, Median: $median, StdDev: $stdDev, SNR: $snr")
